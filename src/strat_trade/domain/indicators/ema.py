...
class EmaCalculator:
    """EMA powered by `ta`; output length matches input candle count."""

    __slots__ = ("_period",)

    def __init__(self, period: int) -> None:
        if period < 2:
            raise IndicatorParameterError("EMA `period` must be >= 2.")
        if period > 1000:
            raise IndicatorParameterError("EMA `period` must be <= 1000.")
        self._period = period
    ...
    @classmethod
    def from_params(cls, params: Mapping[str, object]) -> EmaCalculator:
        raw = params.get("period", 20)
        if raw is None:
            period = 20
        elif isinstance(raw, bool) or not isinstance(raw, int):
            raise IndicatorParameterError("EMA `period` must be an integer.")
        else:
            period = raw
        return cls(period)
    ...
    def compute(self, candles: list[Candle]) -> IndicatorSeries:
        ...
        values = _ema_values(closes, self._period)
...
def _ema_values(closes: pd.Series, period: int) -> list[float | None]:
    try:
        trend = import_module("ta.trend")
        ema_indicator = trend.EMAIndicator  # type: ignore[attr-defined]
    except Exception as exc:
        raise IndicatorParameterError(
            "Technical indicators package `ta` is not available in this environment."
        ) from exc
    series = ema_indicator(close=closes, window=period).ema_indicator()
