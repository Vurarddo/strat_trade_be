...
- **Indicator series:** Indicator math lives in pure domain calculators (see `src/strat_trade/domain/indicators/*`). Metadata: `GET /api/v1/indicators/rsi`, `GET /api/v1/indicators/bollinger-bands`. **Computed:** `POST /api/v1/market/indicators` — same `candles` shape as `GET /api/v1/market/candles`, plus `indicators[]` in request order; each output line is a list of `{ open_time, value }` aligned with `candles` (warmup omitted). Examples: `docs/MARKET_INDICATORS_API.md`.
